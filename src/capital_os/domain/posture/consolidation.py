...
def compute_consolidated_posture(payload: dict) -> dict:
        ...
        for leg in legs:
            if leg["direction"] == "in":
                transfer_net_by_entity[leg["entity_id"]] = normalize_amount(
                    transfer_net_by_entity[leg["entity_id"]] + leg["amount"]
                )
            else:
                transfer_net_by_entity[leg["entity_id"]] = normalize_amount(
                    transfer_net_by_entity[leg["entity_id"]] - leg["amount"]
    ...
    for entity_id in selected_entity_ids:
        ...
        transfer_neutral_liquidity = normalize_amount(item["liquidity"] - transfer_net)

        metrics = compute_posture_metrics(
            PostureComputationInputs(
                liquidity=transfer_neutral_liquidity,
                fixed_burn=item["fixed_burn"],
                variable_burn=item["variable_burn"],
                minimum_reserve=item["minimum_reserve"],
                volatility_buffer=item["volatility_buffer"],
            )
        )
        ...
        entities.append(
            {
                ...
                "minimum_reserve": f"{item['minimum_reserve']:.4f}",
        ...
        consolidated_minimum_reserve = normalize_amount(
            consolidated_minimum_reserve + item["minimum_reserve"]
        )
